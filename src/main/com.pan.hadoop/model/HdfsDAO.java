...
public class HdfsDAO {
	...
	public boolean rename(String folder,String newFolder) throws IOException {
		folder=folder.replace(hdfsPath,"");
		newFolder=new File(folder).getParentFile().toString()+"/"+newFolder;
		Path path = new Path(folder);
		Path newPath= new  Path(newFolder);
		FileSystem fs = FileSystem.get(URI.create(hdfsPath), conf);
		boolean flag = false;
		if (fs.exists(path)) {
			flag= fs.rename(path,newPath);
			System.out.println("rename: " + folder+"->"+newFolder);
		}
		fs.close();
		return flag;
	}
